...
- ~~**점진 흡수**: 앱을 하나씩 허브 라우트로 이동해 단일 SPA로 수렴~~ → **폐기(2026-07-14)**. 허브는 각 앱을 흡수하지 않고 **링크만** 건다. 각 앱은 자기 repo·자기 배포 유지.
...
| ~~6~~ | ~~앱 순차 흡수 → 단일 SPA~~ | — | **폐기** — 허브는 링커로 남는다 |
| **∞** | 개발 PM 상설 컨텍스트 유지, 로드맵 갱신 | — | — |

각 STEP은 독립 배포·독립 롤백 가능하게. **실제 진행은 STEP 0~1 후 스코프가 교정돼, 허브(런처+대시보드)와 팀 경제로 방향이 바뀌었다**(문서 상단 참조).
...
- ~~repo 전략~~ → **확정**: `HK_Hub` 신규 모노레포(**npm workspaces**), 기존 5repo는 **이관하지 않음**. 공유가 필요한 앱만 git 서브모듈로 `@hk/shared` 참조(HK_DP 적용).
