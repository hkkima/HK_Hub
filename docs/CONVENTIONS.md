...
- **포인트 → DP**: 본드커브 `i번째 DP = R0 + k·i^exp`. 확정 **R0=20,000 · k=1,000 · exp=2**(2차). ★2026-07-20 R0 10,000→20,000 리베이스 집행★(현금 방파제. k·exp 불변 — 학생 대부분이 1~5 DP 구간이라 R0 가 지배적. 되돌림은 `meta/dpExchange.prevR0`). 주간 리셋, 단방향(DP→포인트 매도 없음). 코드: `dpcurve.js`(`src/domain` ↔ `functions` 바이트 동일 유지).
